...
public class StylePreferences extends SettingsPreferenceFragment {
    ...
    private Preference mDayPref;
    private Preference mNightPref;
    ...
    public void onCreate(Bundle savedInstanceState) {
        ...
        mDayPref = findPreference("theme_day_style");
        mNightPref = findPreference("theme_night_style");
        setupStylePref();
    ...
    private void onAutomagicCompleted(Style style) {
        String styleType = getString(style.isLight() ?
                R.string.style_global_entry_light : (style.isBlack() ?
                R.string.style_global_entry_dark : R.string.style_global_entry_black)).toLowerCase();
    ...
    private void setupStylePref() {
        ...
        switch (preference) {
            ...
            case INDEX_BLACK:
                ...
                break;
            default:
                getPreferenceScreen().removePreference(mDayPref);
                getPreferenceScreen().removePreference(mNightPref);
            case INDEX_TIME:
    ...
    private void applyStyle(Style style) {
        int value = style.isLight() ? INDEX_LIGHT : (style.isBlack() ? INDEX_DARK : INDEX_BLACK);
    ...
    private boolean checkAccentCompatibility(int value) {
        ...
        boolean time = Settings.System.getInt(getContext().getContentResolver(),
                Settings.System.THEME_GLOBAL_STYLE, INDEX_WALLPAPER) == 1;
    ...
    private static final class AutomagicTask extends AsyncTask<Accent, Void, Style> {
        ...
        public Style doInBackground(Accent... accents) {
            ...
            boolean isBlack = UIUtils.isColorBlack(wallpaperColor);
            Accent bestAccent = getBestAccent(accents, wallpaperColor, isLight);

            return new Style(bestAccent, isLight, isBlack);
        }
